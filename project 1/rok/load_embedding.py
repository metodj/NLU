...
def load_embedding(session, vocab, emb, path, dim_embedding, vocab_size):
    ...
    mean = model.wv.vectors.mean(axis=1).mean()
    std = model.wv.vectors.std(axis=1).mean()
    ...
    for tok, idx in vocab.items():
        if tok in model.vocab:
            ...
            external_embedding[idx] = np.random.normal(loc=mean, scale=std, size=dim_embedding)
