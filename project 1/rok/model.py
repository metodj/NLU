...
class Model:
    ...
    def __init__(self, experiment, mode, vocabulary_size, embedding_dim, state_dim, down_state_dim, sent_dim, cont_dim,
        ...
        # Weights
        with tf.name_scope("weight_initialization"):
    ...
    def forward_pass(self, x, y):
        ...
        with tf.name_scope("word_embedding"):
            x_embedded = tf.nn.embedding_lookup(self.embedding_weight, x, name="embedding_lookup")  # 64x29x100

        # RNN PASS
        with tf.name_scope("rnn_forward_pass"):
            losses = []
            for t in range(self.sent_dim - 1):
                ...
                with tf.name_scope("lstm"):
                    if self.experiment in ["A", "B"]:
                        ...
                        logits = tf.matmul(lstm_output, self.output_weight, name="output_multiplication")  # 64x20000
                    elif self.experiment == "C":
                        lstm_output, (state_c, state_h) = self.LSTM(inputs=x_t, state=(state_c, state_h))  # 64x1024
                        down_logits = tf.matmul(lstm_output, self.down_weight, name="down_projection")  # 64x512
                        logits = tf.matmul(down_logits, self.output_weight, name="output_multiplication")  # 64x20000
        ...
        with tf.name_scope("perplexity"):
            ...
            perplexities = tf.exp(tf.divide(sum_neg_log_prob, sent_length), name="perplexity_exponent_mean")  # 64x1
            mean_perplexity = tf.reduce_mean(perplexities, name="mean_perplexity")  # 1x1
    ...
    def generator(self, x):
        ...
        x = tf.multiply(x, sentence_mask, name="masking") + self.pad_idx * (1 - sentence_mask)
        ...
        with tf.name_scope("rnn_forward_pass"):
            ...
            for t in range(self.cont_dim + 1):
                ...
                with tf.name_scope("lstm"):
                    ...
                    prediction = tf.argmax(logits, axis=1, name="argmax_logits")  # 64x1
