...
def data_to_bert_tutorial(data_path):
  ...
  processed_data = pd.DataFrame(columns=['story', 'label'])
  for _, row in our_data.iterrows():
    sample1 = {'story': row["4_sentences"] + " " + row["RandomFifthSentenceQuiz1"], 'label': 1 if row['AnswerRightEnding']==1 else 0}
    sample2 = {'story': row["4_sentences"] + " " + row["RandomFifthSentenceQuiz2"], 'label': 1 if row['AnswerRightEnding']==2 else 0}
